...
const GeoMap = ({
  ...
  useEffect(() => {
    ...
    g.selectAll("path")
      ...
      .on(
        "mouseover",
        function (this: SVGPathElement, event: MouseEvent, d: GeoFeature) {
          const id = d.properties[idField] || d.id
          d3.select(this).attr(
            "fill",
            id === highlightedAreaId ? hoverHighlightFill : hoverFill
          )
      ...
      .on(
        ...
        function (this: SVGPathElement, event: MouseEvent, d: GeoFeature) {
          ...
          d3.select(this).attr(
            "fill",
            id === highlightedAreaId ? highlightFill : defaultFill
          )
          setTooltipData(null)
        }
      )
